...

{
  ...
  "segments": [ { "name", "share_pct": <number>, "dimension": "product|application|process|composition|channel|region|context", "note", "source": {"label","url","snippet"} } ],
...
- SEGMENTS MUST BE LIKE-FOR-LIKE PER CUT — this is critical. Every segment is one slice of a SINGLE segmentation cut, and you MUST tag each with an explicit "dimension": "product" (product FORM — e.g. flat / long / structural / crude / finished / semi-finished / alloy), "application" (END-USE sector that consumes it — e.g. construction & infrastructure, engineering & packaging, automobile, appliances, capital goods), "process" (production route — e.g. BF-BOF / EAF / induction / primary vs secondary), "composition", "channel", "region", or "context". Within ONE dimension the shares must share the SAME basis and the SAME year, and should sum to roughly 100%. NEVER mix an end-use split with a product-form split in the same cut (that is what makes shares total >100%), and NEVER mix years within a cut. If the facts support two different cuts (e.g. one by product form AND one by end-use), emit BOTH sets of segments, each tagged with its own "dimension", so they render as separate like-for-like breakdowns.
- "highlights" is a flexible strip of the STANDOUT, industry-SPECIFIC numbers a client would want at a glance that the fixed sections don't already carry — e.g. penetration / adoption %, per-capita or per-unit consumption, export or import share, a notable ratio (like the MDF:plywood mix), capacity utilisation, a price or realisation trend, replacement-cycle years. Include up to ~8, each with a short "label", a "value", and a "source". EVERY KPI MUST BE THE MOST RECENT figure for that metric — prefer a ${YEAR} or ${YEAR - 1} value and set "year" to it; do NOT include a KPI whose only value is an old vintage when a newer one exists, and never let one KPI's year lag the others without reason. When the facts give the SAME metric across multiple years, add a "series" of {year,value} (oldest→newest) and set "trend" to up/down/flat (the latest point in the series is what the tile shows). Do NOT duplicate the headline market size here. Omit "highlights" entirely if the facts offer nothing beyond the fixed sections. Richer, sourced data is better; only omit what is unsourced or irrelevant.
...
- "players" is the most important section. Capture as many named companies as the facts support. For each company: set "listed" (true/false) and "ticker" when the facts give them; set "segment" when stated; and use "note" to record qualitative leader / ranking positioning a source states (e.g. largest MDF maker in India, market leader in South India, second-largest capacity, among the top three). Mine BOTH the PLAYERS and MARKET SHARE fact groups for this. Do NOT invent or infer per-company "revenue" or "ebitda_margin_pct" — leave those two fields out unless a source explicitly states that company's figure; per-company financial benchmarking is a separate later step, so omitting them is expected and fine.
- "market_share_pct" IS A ${COUNTRY} SHARE ONLY, FROM THE MOST RECENT SOURCE. Set it for a company ONLY when a source gives that company's share of the ${COUNTRY} market, and use the MOST RECENT such figure available (a ${YEAR}/${YEAR - 1} share beats an older one). NEVER put a company's GLOBAL / worldwide market share into "market_share_pct" — that would mix geographies in the ${COUNTRY} market-share picture. A global-only multinational (whose only stated share is worldwide, with no ${COUNTRY} share) may still be listed as a player for context — put its ${COUNTRY} presence in "note" — but OMIT its "market_share_pct". Prefer the company's ${COUNTRY} operating entity / brand over a separate "(global)" listing, and do NOT add a duplicate "(global)" row when the ${COUNTRY} entity is already present.
