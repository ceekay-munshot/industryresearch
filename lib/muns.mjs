...
async function post(path, payload, kind) {
  ...
  // server errors / network). Every other failure returns null so one bad call
  // can never crash the run — the caller just proceeds with fewer sources.
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (attempt < MAX_ATTEMPTS - 1) {
        console.warn(`[muns] ${path} -> request error (attempt ${attempt + 1}/${MAX_ATTEMPTS}): ${err.message} — retrying...`);
        continue;
      }
      console.warn(`[muns] ${path} -> request error: ${err.message}`);
      return null;
    }
  }
  return null;
