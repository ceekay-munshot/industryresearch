...
/**
 * fetch with a few attempts and exponential backoff. Retries transient failures
 * only — throttling (429), server errors (5xx) and network-level errors. Returns
 * { res, text } on any HTTP response (ok or not) and { res: null, text:'', error }
 * when every attempt threw at the network level. Never throws itself, so a caller
 * can always fall through to `return null` and keep the run alive.
 *
 * `label` is used only for logs — pass the TARGET url, never an api url that
 * carries a token in its query string (e.g. Scrape.do).
 */
async function fetchWithRetry(url, options, label, maxAttempts = 3) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      if (!res.ok && (res.status === 429 || res.status >= 500) && attempt < maxAttempts - 1) {
        console.warn(`[scrape] ${label} -> HTTP ${res.status} (attempt ${attempt + 1}/${maxAttempts}) — retrying...`);
        continue;
      }
      return { res, text };
    } catch (err) {
      ...
      if (attempt < maxAttempts - 1) {
        console.warn(`[scrape] ${label} -> error (attempt ${attempt + 1}/${maxAttempts}): ${err.message} — retrying...`);
        continue;
      }
    }
  }
  return { res: null, text: '', error: lastErr };
}
...
export async function firecrawlScrape(url) {
  ...
  const { res, text: bodyText, error } = await fetchWithRetry(
  ...
  if (!res) {
    console.warn(`[scrape] firecrawl ${url} -> error: ${error ? error.message : 'no response'}`);
    return null;
  }
  if (!res.ok) {
...
export async function scrapedoScrape(url) {
  ...
  // NOTE: token is in the query string — never log this URL. fetchWithRetry is
  // given the target url (not `api`) as its log label so the token never leaks.
  const api = `https://api.scrape.do/?token=${key}&url=${encodeURIComponent(url)}&render=true`;
  const { res, text: html, error } = await fetchWithRetry(api, {}, `scrapedo ${url}`);
  if (!res) {
    console.warn(`[scrape] scrapedo ${url} -> error: ${error ? error.message : 'no response'}`);
    return null;
  }
  if (!res.ok) {
...
export async function mistralOCR(pdfUrl) {
  ...
  if (!res) {
    console.warn(`[scrape] mistral-ocr ${pdfUrl} -> error: ${error ? error.message : 'no response'}`);
    return null;
  }
  if (!res.ok) {
