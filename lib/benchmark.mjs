...
/** Pull the largest financial-looking text block out of an (undocumented) DRHP
 *  response — a restated-financials / prospectus body the extractor can read. Only
 *  returns text that is both substantial AND contains financial vocabulary, so a
 *  stray description or URL list is never mistaken for financials. Pure. */
export function drhpFinancialsText(data) {
  if (!data) return '';
  const FIN = /(revenue|sales|profit|ebitda|operating margin|balance sheet|cash flow|financial year|fiscal|restated|₹|rs\.?\s*(cr|crore|lakh)|total income)/i;
  let best = '';
  const consider = (s) => { if (typeof s === 'string' && s.length >= 400 && s.length > best.length && FIN.test(s)) best = s; };
  const walk = (node) => {
    if (node == null) return;
